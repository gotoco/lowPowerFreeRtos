...
int main(void)
{
	...
	struct acc_config_t * acc_conf = (struct acc_config_t*)malloc(sizeof(struct acc_config_t) );

	config->driver_name = name;
	config->driver_strategy = SEARCH;

	acc_conf->driver_name="MMA9953L";
	acc_conf->user_height=175;
	acc_conf->user_weight=80;
	acc_conf->Gender = Male;

    struct driver_t * drv = new_driver(config);
    struct acc_t* acc_drv=new_acc(acc_conf);

	drv->init_driver(drv, 2, 2);
	drv->flush_buffer(drv);

	acc_drv->acc_Init(acc_drv, 0, acc_conf);
