...
#define RTC_CR_DBP_BB                (PERIPH_BB_BASE + (CR_OFFSET * 32) + (DBP_BitNumber * 4))
...
#define RTC_CSR_EWUP_BB              (PERIPH_BB_BASE + (CSR_OFFSET * 32) + (EWUP_BitNumber * 4))
...
void PWR_RTCAccessCmd(FunctionalState NewState)
{
  ...
  *(__IO uint32_t *) RTC_CR_DBP_BB = (uint32_t)NewState;
}
...
void PWR_WakeUpPinCmd(uint32_t PWR_WakeUpPin, FunctionalState NewState)
{
  ...
  tmp = RTC_CSR_EWUP_BB + PWR_WakeUpPin;
...
void PWR_EnterSleepMode(uint32_t PWR_Regulator, uint8_t PWR_SLEEPEntry)
{
  ...
  assert_param(PWR_IS_PWR_SLEEP_ENTRY(PWR_SLEEPEntry));
...
void PWR_EnterSTOPMode(uint32_t PWR_Regulator, uint8_t PWR_STOPEntry)
{
  ...
  /* Check the parameters */
  assert_param(IS_PWR_REGULATOR(PWR_Regulator));
  assert_param(PWR_IS_PWR_STOP_ENTRY(PWR_STOPEntry));
