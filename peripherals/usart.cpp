...
/*---------------------------------------------------------------------------------------------------------------------+
 | global variables
 +---------------------------------------------------------------------------------------------------------------------*/
xSemaphoreHandle xSemaphoreForBLE_port;
...
enum Error usartInitialize(void)
{
	...
	gpioConfigurePin(USARTx_CT_GPIO, USARTx_CT_PIN, USARTx_CT_CONFIGURATION);
	gpioConfigurePin(USARTx_RT_GPIO, USARTx_RT_PIN, USARTx_RT_CONFIGURATION);
	...
	USARTx->CR3 = USART_CR3_DMAT | USART_CR3_CTSE | USART_CR3_RTSE;  //Set hardware flow controll
	...
	ret = xTaskCreate(_rxTask, (signed char* )"USART RX", USART_RX_STACK_SIZE,
			NULL, USART_RX_TASK_PRIORITY+1, NULL);

	error = errorConvert_portBASE_TYPE(ret);

    vSemaphoreCreateBinary(xSemaphoreForBLE_port);
 ...
 * \param [in] ticks_to_wait is the amount of time the call should block while waiting for the operation to finish,
 * use portMAX_DELAY to suspend
 *
 * \return ERROR_NONE on success, otherwise an error code defined in the file error.h
 */
enum Error usartSendString(const char *string, portTickType ticks_to_wait)
{
	...
	return putMsgToUartQueue(string, length, ticks_to_wait);
}

/**
* \brief Adds byte array to UART TX queue.
*
* Adds byte array to UART TX queue. Calling task should have read-only access to shared_data section.
*
* \param [in] string is the pointer to zero terminated string
* \param [in] ticks_to_wait is the amount of time the call should block while waiting for the operation to finish,
* use portMAX_DELAY to suspend
...
enum Error usartSendBytes(const char *data, size_t length, portTickType ticks_to_wait)
{
    ...
    return putMsgToUartQueue(data, length, ticks_to_wait);
}

/**
* \brief
*
* Adds one Message data to UART TX queue. Calling task should have read-only access to shared_data section.
*/
...
//					0);
		...
		if (message.status == RX_STATUS_HAD_CR_LF) {  // is the message complete (terminated with "\r\n" sequence)?
            ...
            enum Error error = BLE_commandProcessInput(_inputBuffer, _outputBuffer, _OUTPUT_BUFFER_SIZE);	// process input
